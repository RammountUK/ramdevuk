...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  var variantMeta = buildVariantMeta(data.metafields);
  var productJson = buildBISJson(data);
  // Add Custom class
  var soldOutClass = soldOut ? boostPFSTemplate.soldOutClass : '';
  var saleClass = onSale ? boostPFSTemplate.saleClass : '';
  itemHtml = itemHtml.replace(/{{soldOutClass}}/g, soldOutClass);
  itemHtml = itemHtml.replace(/{{saleClass}}/g, saleClass);
  itemHtml = itemHtml.replace(/{{notifyClass}}/g, variantMeta.dataClass);
  itemHtml = itemHtml.replace(/{{itemData}}/g, JSON.stringify(productJson));
  ...
  // Add Metafields
  itemHtml = itemHtml.replace(/{{itemMetafield}}/g, buildMetafields(data.metafields));
  //itemHtml = itemHtml.replace(/{{ball_and_arm}}/g, firstVariant.metafields.variant_attrs.ball_and_arm);
  // Add Price
  ...
  itemHtml = itemHtml.replace(/{{itemTitle}}/g, data.title.replace(/[®]/g, '<sup>$&</sup>'));
  itemHtml = itemHtml.replace(/{{itemHandle}}/g, data.handle);
  itemHtml = itemHtml.replace(/{{itemVendorLabel}}/g, data.vendor);
  itemHtml = itemHtml.replace(/{{itemUrl}}/g, Utils.buildProductItemUrl(data));
  itemHtml = itemHtml.replace(/{{itemSku}}/g, firstVariant.sku);
  itemHtml = itemHtml.replace(/{{itemId}}/g, firstVariant.id);
...
function buildBISJson(data){
  ...
  //sample -> {"id":118958653449,"handle":"hamm2","tags":[],"available":false,"variants":[{"id":1556361314313,"title":"Small","available":false,"inventory_quantity":0,"inventory_management":"shopify"},{"id":1556361347081,"title":"Large","available":false,"inventory_quantity":0,"inventory_management":"shopify"}]}
  return product
}
function buildVariantMeta(metafields){
  data = {

  };
  metafields.map((meta) => {
    if(meta.key == 'notify'){
      data.dataClass = JSON.parse(meta.value).NOTIFY_ME == "true" ? 'show-notify ' : 'hide-notify ';
      data.dataClass += JSON.parse(meta.value).HIDE_PRICE == "true" ? 'hide-price ' : 'show-price ';
    }
  })
  return data;
...
function buildMetafields(metafields){
  var html = '';
  data = {
  ...
  metafields.map((meta) => {
    ...
    if(meta.key == 'ball_and_arm'){


      html += '<div class="metafields-container">'
      if(JSON.parse(meta.value).ball_size == "A"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/ball-icon.jpg?v=1656006405" alt="ball size icon" />'
        html += '<span> A SIZE </span>'
        html += '</div>'
      }
      if(JSON.parse(meta.value).ball_size == "B"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/ball-icon.jpg?v=1656006405" alt="ball size icon" />'
        html += '<span> B SIZE </span>'
        html += '</div>'
      }
      if(JSON.parse(meta.value).ball_size == "C"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/ball-icon.jpg?v=1656006405" alt="ball size icon" />'
        html += '<span> C SIZE </span>'
        html += '</div>'
      }
      if(JSON.parse(meta.value).ball_size == "D"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/ball-icon.jpg?v=1656006405" alt="ball size icon" />'
        html += '<span> D SIZE </span>'
        html += '</div>'
      }
      if(JSON.parse(meta.value).ball_size == "E"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/ball-icon.jpg?v=1656006405" alt="ball size icon" />'
        html += '<span> E SIZE </span>'
        html += '</div>'
      }
      //add metafield logic
      ...
      if(JSON.parse(meta.value).socket_arm_length == "Long"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/arm-icon.jpg?v=1656006390" alt="arm length icon" />'
        html += '<span> LONG ARM </span>'
        html += '</div>'
      }
      if(JSON.parse(meta.value).socket_arm_length == "Short"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/arm-icon.jpg?v=1656006390" alt="arm length icon" />'
        html += '<span> SHORT ARM </span>'
        html += '</div>'
      }
      if(JSON.parse(meta.value).socket_arm_length == "Medium"){
        html += '<div class="icon">'
        html += '<img src="https://cdn.shopify.com/s/files/1/0593/5346/4982/files/arm-icon.jpg?v=1656006390" alt="arm length icon" />'
        html += '<span> MED ARM </span>'
        html += '</div>'
      }
...
function buildPrice(data) {
  ...
  if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
    boostPFSThemeConfig.custom.show_price && data.price_min) {
...
//   ProductList.prototype.buildProductList = function() {
//     console.log('buildProductList');
//     var productEls = [];
//     // Get all product item (DOM element)
//     this.productItems.forEach((productitem) => {
//       productEls.push(productitem.$element);
//     });
//     console.log(productEls);
//     // Remove the sort by attribute of product list element
//     this.$element.removeAttr('data-boost-sort');
//     // Clear the HTML content of product list if the pagination is default OR the event type is not page
//     if (Utils.isDefaultPaginationType() || (!Utils.isDefaultPaginationType() && this.eventType != 'page')) {
//       this.$element.html('');
//       jQ(window).off('scroll');
//     }
//     // Prepend the product result if the event is load previous
//     if (Utils.isLoadPreviousPagePaginationType() && parseInt(window.sessionStorage.getItem(this.settings.sessionStoragePreviousPageEvent))) {
//       console.log('2');
//       var reverseProductEls = [];
//       productEls.forEach(element => reverseProductEls.push(element));
//       reverseProductEls.reverse();
//       reverseProductEls.forEach(element => this.$element.prepend(element));
//     } else {
//       console.log('1', this.$element, productEls);
//       productEls.forEach(element => this.$element.append(element))
//     }

//     // Call 3rd app integration code
//     Integration.call3rdAppFunction(this.data); // Old integration method
//     Integration.call3rdIntegrationFunc(this.data);

//     // Re-call afterRender to add additional feature for product list
//     if (this.settings.loadProductFromLiquid && this.settings.loadProductFromLiquidType == ProductList.Enum.loadProductType.AJAX) {
//       this.afterRender();
//     }
//   }
...
BoostOTP.AjaxCartItem.prototype.getTemplate = function() {
        return`
            ...
            <div class="boost-pfs-minicart-item-details">
                <div class="boost-pfs-minicart-item-details-inner">
                    <div class="boost-pfs-minicart-item-group-title">
                        ...
                        <p class="boost-pfs-filter-product-item-vendor">{{sku}}</p>
...
BoostOTP.AjaxCartItem.prototype.compileTemplate = function() {
  var image = this.data.image ? this.data.image :
			(this.data.featured_image && this.data.featured_image.url) ? this.data.featured_image.url : boostPFSConfig.general.no_image_url;

		return this.getTemplate()
            .replace(/{{sku}}/g, this.data.sku)
			.replace(/{{variantUrl}}/g, this.data.url)
			.replace(/{{productTitle}}/g, this.data.product_title)
			.replace(/{{productId}}/g, this.data.product_id)
			.replace(/{{vendor}}/g, this.data.vendor)
			.replace(/{{variantImage}}/g, image)
			.replace(/{{productOptions}}/g, this.getProductOptions(this.data))
			.replace(/{{variantId}}/g, this.data.variant_id)
			.replace(/{{quantity}}/g, this.data.quantity)
			.replace(/{{price}}/g, Utils.formatMoney(this.data.price))
			.replace(/{{subTotalPrice}}/g, Utils.formatMoney(this.data.line_price));
}
...
/*End boost Custom */
  ...
  /* start-boost-custom */
  /* #boost-135627: customize multi-level filter option to correlate items */
  Filter.prototype.modifyData = function(data){
      if (!data) data = this.data;
      if (data.filter) {
        for (var i = 0; i < data.filter.options.length; i++) {
          if (data.filter.options[i].filterType == 'multi_level_tag') {
              data.filter.options[i].selectType = 'multiple'
          }
        }
      }
  ...
  FilterOptionItemMultiLevelTag.prototype.onClick = function(event) {
    if (event) {
      event.preventDefault();
    }
    if (!this.isDisabled()) {
      if (this.parent.filterType == FilterOptionEnum.FilterType.MULTI_LEVEL_TAG ||
      this.parent.parent.filterType == FilterOptionEnum.FilterType.MULTI_LEVEL_TAG ||
      this.parent.parent.parent.filterType == FilterOptionEnum.FilterType.MULTI_LEVEL_TAG) {
        var level = this.level
        var object = this
        while (level >= 1) {
          if (typeof object.onApplyFilter == "function"){
            if (object != this) {
              object.onApplyFilter(this.isSelected)
            } else {
              object.onApplyFilter()
            }
          }
  ...
  FilterOptionItemMultiLevelTag.prototype.onApplyFilter = function(value) {
  	// If this is selected, and select type is single, do nothing
  	if (this.isSelected && this.filterOption.selectType == FilterOptionEnum.SelectType.SINGLE) {
  		// Expand the children
  		if (this.children && this.children.length > 0) {
  			this.$itemElement.addClass('boost-pfs-open');
  			FilterOptionItemMultiLevelTag.setLevelOpenStateData(this, true);
  		}
  		return;
  	}

  	// Expand the children after filtering
  	if (this.children && this.children.length > 0) {
  		FilterOptionItemMultiLevelTag.setLevelOpenStateData(this, true);
  	}

  	var filterOptionId = this.filterOption.filterOptionId;
  	...
  	if (this.filterOption.selectType == FilterOptionEnum.SelectType.SINGLE) {
  	...
  	FilterApi.setParam(filterOptionId + '_and_condition', this.filterOption.useAndCondition && values.length > 0 ? true : null);
